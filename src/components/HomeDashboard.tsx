    ...
    return (
        <>
            <div id="aboutus" className="home-1">
                <div className=" pt-16 pb-10 w-full md:min-h-[800px] min-h-[1000px] flex flex-col items-center justify-between text-center backdrop-blur-none bg-black/30">
                    <div className="flex flex-col items-center">
                        <h1 className="font-bold text-[white] text-6xl">CIVIL CLAIM LAWYER</h1>
                        <p className="pt-3 font-bold text-2xl text-[white]">Referral Portal For Injustice</p>
                    </div>
                    <div className="flex flex-col items-center">
                        <h1 className="font-bold text-[40px] text-[white] mb-5">Free No Cost Consultation</h1>
                        <h2 className="text-[26px] text-[white]">Victim of Injustice??? Speak to a top Lawyer today</h2>
                    </div>
                    <div className="flex flex-col">
                        <div className="flex flex-col xl:w-[1050px] xl:h-[245px] bg-transparent rounded-[6px] mt-4 gap-5">
                            <h2 className="pl-5 mb-3 text-4xl text-[white]">What's my claim worth?</h2>
                            <div className="grid w-full grid-cols-4 mx-auto md:grid-cols-8">
                                {
                                    accident_style.map((item, index) => {
                                        return <div key={item.name + index} className={`${accidentId === index ? "grayscale-0" : "grayscale"} hover:scale-125 hover:grayscale-0 flex flex-col items-center cursor-pointer`} onClick={() => setAccidentId(index)}>
                                            ...
                                            <div className="text-[14px] text-[#f7901e]">{item.name}</div>
                                        </div>
                                    })
                                }
                            </div>
